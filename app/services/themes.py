...
def load_themes() -> list[Theme]:
    """data/themes.json を読み込む。破損・欠如時は空リスト + warning。"""
    path = _themes_path()
    if not path.exists():
        return []
    try:
        ...
        if not isinstance(raw, list):
            return []
        return [Theme.model_validate(t) for t in raw]
    except Exception as e:
        logger.warning(f"themes.json 読み込み失敗: {e}")
        return []
...
async def generate_keywords(theme_name: str) -> list[str]:
    ...
    try:
        ...
        logger.error(f"テーマ '{name}' のキーワード生成失敗: {e}")
        return []
