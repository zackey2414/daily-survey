...
| LLM API | **Google Gemini API** | `gemini-2.5-flash` を基本モデルとし、要約など重い処理には `gemini-2.5-pro` を検討 |
| スケジューラ | **APScheduler**（FastAPI 組み込み）または **cron**（Docker 内） | JST 09:00 実行 |
| 通知 | SMTP（メール） | 収集完了時 |

### 2.3 Gemini API モデル選定方針

- **要約処理**（個別記事の要約・一面まとめ生成）: `gemini-2.5-pro`（精度重視）
- **チャット応答**: `gemini-2.5-flash`（コスト効率重視、高速）
- モデル名は設定ファイル（`.env`）で個別に切り替え可能にすること
...
- 使用モデル: Gemini API（`gemini-2.5-pro`）
- 出力言語: **日本語**
- 論文の場合に含める内容:
  - 研究概要（何をしたか）
  - 新規性・貢献（既存手法との差異）
...
- 使用モデル: Gemini API（`gemini-2.5-flash`）
- 入力: 全カテゴリの要約データ（JSON）
...
- URL: `/summaries`
- 2つのビューをボタンで切り替え:
  1. **最新3日分ビュー**: 直近3日分の一面まとめを縦に並べて表示
  2. **全件一覧ビュー**: カレンダー形式 + リスト形式（トグル可）で全日付を表示
- 各日付のサマリーをクリックすると、その日のまとめページに遷移（`/summaries/YYYY-MM-DD`）
...
── 最新3日分ビュー ──────────────────────────────────
2026-03-03  本日の AI 動向まとめ
（Markdown 内容の冒頭プレビュー + 「続きを読む」）
