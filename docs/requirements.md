...
| LLM API | **Google Gemini API** | `gemini-2.0-flash` を基本モデルとし、要約など重い処理には `gemini-2.0-pro` を検討 |
| スケジューラ | **APScheduler**（FastAPI 組み込み）または **cron**（Docker 内） | JST 09:00 実行 |
| 通知 | SMTP（メール） | 収集完了時 |

### 2.3 Gemini API モデル選定方針

- **基本モデル**: `gemini-2.5-flash`（コスト効率重視、高速・高精度）
- 全処理（個別要約・一面まとめ生成・チャット応答）で統一して `gemini-2.5-flash` を使用する
- モデル名は設定ファイル（`.env`）で切り替え可能にすること
